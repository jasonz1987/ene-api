 ...
 * @Command
 */
class CheckStake extends HyperfCommand
{
    /**
    ...
    public function __construct(ContainerInterface $container)
    {
        ...
        parent::__construct('check:stake');
    }
    ...
    public function handle()
    {
        ...
        if ($latest_block_number == null) {
            \App\Utils\Log::get()->error("【扫描LP质押】获取最新区块失败");
            return;
        }

        \App\Utils\Log::get()->info(sprintf("【扫描LP质押】最新区块号:%s", $latest_block_number));
        ...
        \App\Utils\Log::get()->info("【扫描LP质押】开始扫描...");
        ...
        $web3->getEth()->getLogs([
            ...
            function ($err, $result) use (&$eventLogData, &$fromBlock, $tx_ids, $queueService, &$latest_block_number, $contract, $contractAddress, $ethabi, $eventParameterTypes, $eventParameterNames, $eventIndexedParameterTypes, $eventIndexedParameterNames, $numEventIndexedParameterNames) {
                if($err !== null) {
                    \App\Utils\Log::get()->error(sprintf("【扫描LP质押】扫描失败:%s", $err->getMessage()));
                    throw new \Exception($err->getMessage());
                }

                \App\Utils\Log::get()->info(sprintf("【扫描LP质押】扫描的交易数:%s", count($result)));

                foreach ($result as $object) {

                    \App\Utils\Log::get()->info(sprintf("【扫描LP质押】扫描的交易:%s", $object->transactionHash));

                    if (in_array($object->transactionHash, $tx_ids)) {
                        \App\Utils\Log::get()->info(sprintf("【扫描LP质押】交易已存在，跳过:%s", $object->transactionHash));
                        continue;
                    }
                    ...
                    try {
                        ...
                        Db::rollBack();
                        \App\Utils\Log::get()->error(sprintf("【扫描LP质押】更新算力失败:%s",  $e->getMessage()));
                        throw new \Exception("【扫描LP质押】更新算力失败：" . $e->getMessage());
                    }
                }
